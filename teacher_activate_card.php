...
if($account_ID > 0)
{
	if($barcode != "")
	{
		...
		if(mysql_num_rows($result) > 0)
		{
			...
			exit_error("Card Already In Use", "The card you have entered is already in use by " . $record->name);
		}
		else
		{
			$sql = "
			...
			header("location:" . $db->url_root . "/teacher_reset_pin.php/" . $barcode);
			exit();
		}; // end if
	}; // end if
	...
	<script language="javascript">
		function submit_barcode()
		{
			...
			{
				document.location.href = "<?= $db->url_root; ?>/teacher_activate_card.php/<?= $account_ID; ?>/" + document.getElementById('barcode_input').value;
			}
			else
			{
	...
	<div class="dialog_instructions">Enter new library card for</div>
