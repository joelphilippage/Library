...
// This file has all the tools needed to create PDF files on the fly.

class class_pdf_maker extends FPDF
{
	var $flash_x_offset = 0;
	var $flash_y_offset = 0;
	var $flash_x_pos = 0;
	var $flash_y_pos = 0;
	var $flash_f_size = 0;
	var $flash_font = "";
	var $flash_fonts = array();
	var $flash_font_size_conversion = 72;
	var $flash_align = array('right' => "R", 'left' => "L", 'center' => "C", 'justify' => "J");
	...
	function offset($x, $y)
	{
		$this->flash_x_offset = $x;
		$this->flash_y_offset = $y;
	} // end method
	...
	function rounded_rect($x, $y, $w, $h,$r, $style = '')
	{
		$this->rect_rounded($y, $x, $w, $h,$r, $style);
	} // end method

	function set_font($font, $size = 0)
	{
		$this->flash_font = $font;
		if(! array_key_exists($this->flash_font, $this->flash_fonts))
		{
			parent::SetFont($this->flash_font,'', $size);
			$this->flash_fonts[$this->flash_font] = TRUE;
		}; // end if
	...
	function text($x, $y, $size, $text, $align = "left", $font = "")
	{
		if($font == "")
		{
			if($this->flash_font == "")
		...
		$this->set_font($this->flash_font, $size);

		$width = $this->WriteHTML($size, $text, TRUE);
		$y_pos = $y;
		$x_pos = $x;
		switch($align)
		{
		...
		parent::SetXY($this->flash_y_offset + $y_pos - 0.04, $this->flash_x_offset + $x_pos + 0.01);
		$this->WriteHTML($size, $text);
		// parent::Text($this->flash_y_offset + $y_pos, $this->flash_x_offset + $x_pos + $size, $text);
		return $width;
	} // end method

	function text_wrap($x, $y, $size, $text, $width, $align = "left", $font = "")
	{
		if($font == "")
		{
			if($this->flash_font == "")
		...
		parent::SetXY($this->flash_y_offset + $y - 0.04, $this->flash_x_offset + $x + 0.01);
		parent::MultiCell($width, $size, $text, 0, $this->flash_align[$align], 0);
		
	} // end method

	function WriteHTML($size, $html, $return_width = FALSE)
	{
		$width = 0;
		$html=str_replace("\n",' ',$html);
	...
	function image_jpeg($pathname, $x, $y, $width, $height = NULL)
	{
		if($height === NULL)
		{
			$ih = getimagesize($pathname);
			$ratio = $ih[1] / $ih[0];
			$height = $width * $ratio;
		}; // end if
		parent::Image($pathname, $this->flash_y_offset + $y, $this->flash_x_offset+ $x, $width, $height);
	} // end method

	function new_page($orientation = "P")
	{
		parent::AddPage($orientation);
	} // end method

	function stream($filename = "", $download = FALSE)
	{
		parent::Output($filename, $download);
	} // end method

	function error($message)
	{
		exit("<br><br><b>flash pdf maker error:</b> " . $message);
	} // end method

}; // end class

define('FPDF_FONTPATH', $db->root_dir . "/include/fonts/");
...
$pm = new class_pdf_maker($pdf_orientation);
...
// *** EVERYTHING ABOVE THIS LINE IS THE FLASH SERVICES OVERLAY CODE *************/
// *** EVERYTHING BELOW THIS LINE IS THE ORIGINAL CODE FROM www.fpdf.com *************/
...
function Image($file,$x,$y,$w,$h=0,$type='',$link='')
{
	...
	if(!isset($this->images[$file]))
	{
		...
		if($type=='')
		{
			...
			$type=preg_replace("/\/.*/", "", substr($file,$pos+1));
		}
...
function complete_rotate($angle,$x=-1,$y=-1)
{
    if($x==-1)
        $x=$this->x;
    if($y==-1)
...
function rect_rounded($x, $y, $w, $h,$r, $style = '')
{
	$k = $this->k;
	$hp = $this->h;
	if($style=='F')
