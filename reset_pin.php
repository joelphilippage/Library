...
if($barcode != "")
{
	...
	{
		exit_error("Not Active Student Library Card", "The card you have entered is not an active student library card.");
	}; // end if
...
else
{
	...
	<script language="javascript">
		function submit_barcode()
		{
			...
			{
				document.location.href = "<?= $db->url_root; ?>/reset_pin.php/" + document.getElementById('barcode_input').value;
			}
			else
			{
	...
	<div class="dialog_title">Enter library card number</div>
